...
static void tick_nohz_stop_sched_tick(struct tick_sched *ts)
{
	...
	ts->idle_calls++;
	/* Read jiffies and the time when jiffieunsigned long rcu_delta_jiffies;s were updated last */
	do {
	...
	if ((long)delta_jiffies >= 1) {
		 ...
		 * max_deferement value which weunsigned long rcu_delta_jiffies; retrieved
